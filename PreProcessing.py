...
def processPassengers():
    file = open("rawdata/12abuckets.csv")
    grouped = []
    header1 = []
    header2 = []
    dateBuckets = {}
    for line in file:
        ...
        if (sp[0] == ""): #first line
            ...
            date = sp[1].split('/')[0]
            time = sp[-1].rstrip()
            passengers = list(map(lambda x: int(x), sp[3:20]))
    ...
    outputFile = open("processeddata/12aproc.csv", 'w')

    #write headers
    outputFile.write(','.join(header1[2:20]) + '\n')
    outputFile.write(','.join(header2[2:20]) + '\n')
